; This is ABM version 1 (three types developers)
...
to load-input
  ...
  foreach sort patches [ ?1 -> ask ?1 [set field-land-adm file-read] ]
  file-close

  ; Load land use in 1994
  file-open "1994_jma_rsmpl.txt"
  foreach sort patches [ ?1 -> ask ?1 [set field-land-use-ori file-read] ]
  file-close

  ; Load distance from toll road
  file-open "jma_tollroad_buff.txt"
  foreach sort patches [ ?1 -> ask ?1 [set field-dist-road file-read] ]
  file-close

  ; Load land-values
  file-open "jma_land_val_v5.txt"
  foreach sort patches [ ?1 -> ask ?1 [set field-land-value-ori file-read] ]
  file-close
